...
public class B_Generateur_ANFR_Diff {
    ...
    private SQLiteConnexion dbAa;
    private SQLiteConnexion dbHt_1;                 //doit être le plus récent
    private SQLiteConnexion dbHt_2;

    private String file1, file2;
    
    private final String JS_FILENAME = "diff_1p_op.js";
    private final String SQL_HISTO_PATH = Main.ABS_PATH + File.separator+"SQL"+File.separator+"ANFR"+File.separator;
    ...
    private void generateur() {
        if (checkBases()) {
            ...
            dbHt_1 = new SQLiteConnexion();
            dbHt_1.dbPath = SQL_HISTO_PATH;         // override path
            dbHt_1.restore(file1);

            dbHt_2 = new SQLiteConnexion();
            dbHt_2.dbPath = SQL_HISTO_PATH;         // override path
            ...
            File f = new File(Main.ABS_PATH+"/Generated/"+ JS_FILENAME);
            f.delete();
            // écrire l'entête du fichier JS
            writeFile(JS_FILENAME,"//Generated "+Main.dateLog()+" "+Main.timeLog());
            writeFile(JS_FILENAME,"var addressPoints = [");
            ...
            /* ------ Statistiques ------- */
            writeFile(JS_FILENAME,"];\n"); //dernière ligne !
            writeFile(JS_FILENAME, "var update = ['"+ Main.epoch2Date(Main.timestampMilis()/1000)+"'];");
            writeFile(JS_FILENAME, "var dataset = ['"+ sqlVersion+"'];");
            writeFile(JS_FILENAME,"var news = ['"+news+"'];");
            writeFile(JS_FILENAME,"var acti = ['"+acti+"'];");
            writeFile(JS_FILENAME,"var news5 = ['"+news5+"'];");
            writeFile(JS_FILENAME,"var acti5 = ['"+acti5+"'];");
            writeFile(JS_FILENAME,"var supp = ['"+dbAa.numberRowsQ("SELECT COUNT(*) AS rowcount FROM Nouvelles WHERE xG = 4 AND Flag = 'N'")+"'];");
            writeFile(JS_FILENAME,"var off = ['"+dbAa.numberRowsQ("SELECT COUNT(*) AS rowcount FROM Nouvelles WHERE xG = 4 AND Flag = 'A'")+"'];");
            writeFile(JS_FILENAME,"var supp5 = ['"+dbAa.numberRowsQ("SELECT COUNT(*) AS rowcount FROM Nouvelles WHERE xG = 5 AND Flag = 'N'")+"'];");
            writeFile(JS_FILENAME,"var off5 = ['"+dbAa.numberRowsQ("SELECT COUNT(*) AS rowcount FROM Nouvelles WHERE xG = 5 AND Flag = 'A'")+"'];");
    ...
    protected void writeMonoPlmn(int ajout, int anfrId, int plmn) {
        ...
        try {
            ...
            writeFile(JS_FILENAME,"[" + lat + ", " + lon + ", " + plmn+", "+anfrId + ", \"" + anfrData + "\", \""+ systDecl  + "\", \""+ systActi + "\", "+ajout+"],");
            rs.close();
    ...
    protected void writeOm(int ajout, int anfrId, int groupe) {
        ...
        try {
            ...
            writeFile(JS_FILENAME,"[" + lat + ", " + lon + ", " + 9999+""+groupe +", "+anfrId + ", \"" + anfrData + "\", \""+ systDecl  + "\", \""+ systActi + "\", "+ajout+"],");
            rs0.close();
        } catch (SQLException e) {
            e.printStackTrace();
        }
